...
function SettlementsContent() {
  ...
  const selectChange = async (changeId: number) => {
    ...
    try {
      ...
      setError(null);
      setSelectedChange(await getBudgetChange(roomId, changeId));
    } catch (caughtError) {
      setError(
        caughtError instanceof Error
          ? caughtError.message
  ...
  if (error && !selectedChange) {
    return <ErrorState message={error} onRetry={() => void loadChanges()} />;
  }
      ...
      <div className="mt-8 grid gap-7 lg:grid-cols-[minmax(0,1.3fr)_minmax(280px,0.7fr)]">
        <section className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-5">
            <h2 className="font-bold text-zinc-900">정산 목록</h2>
            <span className="text-sm text-zinc-500">총 {changes.length}건</span>
          </div>
                ...
                <li key={change.id}>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm text-zinc-500">
                        {change.userName} · {formatDate(change.processedAt)}
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      ...
                      <p className="mt-1 text-xs text-zinc-500">{change.type}</p>
                    </div>
                  </button>
                </li>
              ))}
        ...
        <aside className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
          <h2 className="font-bold text-zinc-900">정산 상세</h2>

          {detailLoading ? (
            <p className="mt-6 text-sm text-zinc-500">상세 정보를 불러오는 중입니다.</p>
          ) : selectedChange ? (
            <dl className="mt-6 space-y-4 text-sm">
              <DetailItem label="정산자" value={selectedChange.userName} />
              <DetailItem label="이메일" value={selectedChange.userEmail} />
              <DetailItem label="신청 금액" value={formatAmount(selectedChange.changeBudget)} />
              <DetailItem label="실제 지출" value={formatAmount(selectedChange.changedBudget)} />
              <DetailItem label="정산 사유" value={selectedChange.reason || "-"} />
              <DetailItem label="처리 일시" value={formatDate(selectedChange.processedAt)} />
            </dl>
          ) : (
...
function LoadingState() {
  return (
    <section className="mx-auto w-full max-w-5xl rounded-2xl bg-white p-8 text-center shadow-sm">
      <p className="text-sm text-zinc-500">정산 내역을 불러오는 중입니다.</p>
    </section>
  );
}
