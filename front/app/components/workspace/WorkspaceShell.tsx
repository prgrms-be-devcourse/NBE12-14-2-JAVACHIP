...
type IconName = "dashboard" | "request" | "settlement" | "members" | "invite" | "approval" | "menu" | "close" | "chevron" | "back";

const navigation: { label: string; href: string; icon: IconName; badge?: string }[] = [
  { label: "대시보드", href: "/dashboard", icon: "dashboard" },
  { label: "예산 신청", href: "/budget-requests", icon: "request" },
  { label: "정산하기", href: "/settlements", icon: "settlement" },
  { label: "멤버", href: "/members", icon: "members" },
  { label: "초대하기", href: "/invitations", icon: "invite" },
  { label: "승인 관리", href: "/approvals", icon: "approval", badge: "2" },
];

function Icon({ name, className = "" }: { name: IconName; className?: string }) {
  const svgProps = { className: `h-5 w-5 shrink-0 ${className}`, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.8, strokeLinecap: "round" as const, strokeLinejoin: "round" as const, "aria-hidden": true };
...
export default function WorkspaceShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f8f8fb] text-zinc-900">
    <div className="fixed inset-y-0 left-0 z-20 hidden md:block">
      <Sidebar />
    </div>
    
    <header className="sticky top-0 z-10 flex h-16 items-center border-b border-zinc-200 bg-white px-4 md:hidden"><button type="button" aria-label="메뉴 열기" onClick={() => setIsOpen(true)} className="rounded-lg p-2 text-zinc-700 hover:bg-zinc-100"><Icon name="menu" /></button><span className="ml-3 text-sm font-semibold">한양대 사진동아리 렌즈</span></header>

    {isOpen && (
      <div className="fixed inset-0 z-30 md:hidden"><button type="button" aria-label="메뉴 닫기" onClick={() => setIsOpen(false)} className="absolute inset-0 bg-zinc-900/30" /><div className="relative h-full w-72 bg-white shadow-xl"><button type="button" aria-label="메뉴 닫기" onClick={() => setIsOpen(false)} className="absolute right-3 top-4 rounded-lg p-2 text-zinc-600 hover:bg-zinc-100"><Icon name="close" /></button><Sidebar onNavigate={() => setIsOpen(false)} /></div></div>
    )}
